				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case LOWPOWER_CURRENT:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.current_low);
	case HIGHPOWER_CURRENT:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.current_high);
	case NOTIFICATION_RAMP_CONTROL:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.noti_ramp_control);
	case NOTIFICATION_RAMP_UP:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.noti_ramp_up);
	case NOTIFICATION_RAMP_DOWN:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.noti_ramp_down);
	case NOTIFICATION_DELAY_ON:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.noti_delay_on);
	case NOTIFICATION_DELAY_OFF:
		return snprintf(buf, RGB_BUFSIZE, "%d\n", leds_control.noti_delay_off);
	}
