...
static int zram_decompress_page(struct zram *zram, char *mem, u32 index)
{
	...
	cmem = zpool_map_handle(zram->mem_pool, handle, ZPOOL_MM_RO);
	if (size == PAGE_SIZE) {
			  ...
			  u32 index, int offset)
{
	...
	if (unlikely(!zram->table[index].handle) ||
		...
		zram_unlock_table(&zram->table[index]);
		handle_same_page(bvec, zram->table[index].element);
		return 0;
	}
	zram_unlock_table(&zram->table[index]);
...
compress_again:
	 ...
	 */
	if (!handle)
		ret = zpool_malloc(zram->mem_pool, clen,
	...
	cmem = zpool_map_handle(zram->mem_pool, handle, ZPOOL_MM_WO);
	...
	zram_set_obj_size(zram, index, clen);
	zram_unlock_table(&zram->table[index]);
...
static int zram_add(void)
{
	...
	queue_flag_set_unlocked(QUEUE_FLAG_FAST, zram->disk->queue);
