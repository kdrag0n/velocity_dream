...
static int nad_balancer_parse_dt(struct device *dev)
{
	...
	pdata->sleep_items = devm_kzalloc(dev, sizeof(struct nad_balancer_sleep_info), GFP_KERNEL);
		       ...
		       unsigned long long usec)
{
	...
	case PM_EVENT_SUSPEND:
		if ((usec / USEC_PER_MSEC) > psleep->suspend_threshold) {
			panic("NAD_BALANCER: over suspend time! dev:%s, %Ld.%03Ld msecs\n",
				dev_name(dev), usec / USEC_PER_MSEC, usec % USEC_PER_MSEC);
		}
		break;
	case PM_EVENT_RESUME:
		if ((usec / USEC_PER_MSEC) > psleep->resume_threshold) {
			panic("NAD_BALANCER: over resume time! dev:%s, %Ld.%03Ld msecs\n",
				dev_name(dev), usec / USEC_PER_MSEC, usec % USEC_PER_MSEC);
		}
		break;
	default:
