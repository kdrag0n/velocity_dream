				...
				battery->cable_type == SEC_BATTERY_CABLE_PREPARE_WIRELESS_HV)
			{
				if (battery->cable_type != SEC_BATTERY_CABLE_WIRELESS_HV_VEHICLE)
				{
						 ...
						 battery->chg_temp >= battery->pdata->wpc_lcd_on_high_temp_rec))
					{
						...
						battery->chg_limit = true;
						wpc_vout_level = (battery->capacity < 95) ? WIRELESS_VOUT_5V_STEP : WIRELESS_VOUT_10V;
					}
					else if (battery->chg_limit)
					{
						battery->chg_limit = false;
...
static int sec_bat_check_pdic_temp(struct sec_battery_info *battery, int input_current)
{
	if (battery->pdic_ps_rdy && battery->siop_level >= 100)
	{
		...
		if (pd_index < 0)
		{
			if (battery->chg_limit)
			{
				input_current = (input_current > (battery->pdata->nv_charge_power / (pd_list->pd_info[0].input_voltage / 1000))) ? (battery->pdata->nv_charge_power / (pd_list->pd_info[0].input_voltage / 1000)) : input_current;
				pd_index = -1;
			}
			else
			{
...
static void sec_bat_get_charging_current_in_power_list(struct sec_battery_info *battery)
{
	...
	max_charging_current = max_charging_current > battery->pdata->max_charging_current ? battery->pdata->max_charging_current : max_charging_current;
...
static int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	else
	{
		...
		if (is_wireless_type(battery->cable_type))
		{
			...
			{
				charging_current = (charging_current > battery->pdata->swelling_wc_low_temp_current) ? battery->pdata->swelling_wc_low_temp_current : charging_current;
				topoff_current = (topoff_current > battery->pdata->swelling_low_temp_topoff) ? battery->pdata->swelling_low_temp_topoff : topoff_current;
			}
			else if (battery->current_event & SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING)
			{
				charging_current = (charging_current > battery->pdata->swelling_wc_high_temp_current) ? battery->pdata->swelling_wc_high_temp_current : charging_current;
				topoff_current = (topoff_current > battery->pdata->swelling_high_temp_topoff) ? battery->pdata->swelling_high_temp_topoff : topoff_current;
			}
		...
		else
		{
			if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING)
			{
				charging_current = (charging_current > battery->pdata->swelling_low_temp_current) ? battery->pdata->swelling_low_temp_current : charging_current;
				topoff_current = (topoff_current > battery->pdata->swelling_low_temp_topoff) ? battery->pdata->swelling_low_temp_topoff : topoff_current;
			}
			else if (battery->current_event & SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING)
			{
				charging_current = (charging_current > battery->pdata->swelling_high_temp_current) ? battery->pdata->swelling_high_temp_current : charging_current;
				topoff_current = (topoff_current > battery->pdata->swelling_high_temp_topoff) ? battery->pdata->swelling_high_temp_topoff : topoff_current;
			}
			else if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP)
			{
	...
	struct sec_battery_info *battery)
{
	...
	else if (battery->pdata->usb_thermal_source && (battery->usb_temp > battery->temp_highlimit_recovery) && (battery->health == POWER_SUPPLY_HEALTH_OVERHEATLIMIT))
	{
		dev_err(battery->dev,
	...
	struct sec_battery_info *battery)
{
	...
	{
		charging_time = 0xFFFFFFFF - battery->charging_start_time + ts.tv_sec;
	}
	...
	struct sec_battery_info *battery)
{
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		...
		{
			battery->polling_time =
				battery->pdata->polling_time[SEC_BATTERY_POLLING_TIME_SLEEP];
		}
		else
			battery->polling_time =
				battery->pdata->polling_time[battery->status];
	...
	struct sec_battery_info *battery)
{
	...
	if (battery->polling_short &&
		((battery->polling_time /
		  battery->pdata->polling_time[SEC_BATTERY_POLLING_TIME_BASIC]) > battery->polling_count))
...
skip_current_monitor:
	...
	if ((battery->cable_type != SEC_BATTERY_CABLE_NONE) && (battery->cable_type != SEC_BATTERY_CABLE_OTG) && battery->store_mode)
	{
		...
		{
			int chg_mode = battery->misc_event & BATT_MISC_EVENT_UNDEFINED_RANGE_TYPE ? SEC_BAT_CHG_MODE_BUCK_OFF : SEC_BAT_CHG_MODE_CHARGING_OFF;
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if (battery->wc_status && battery->wc_enable)
	{
		...
		if (battery->wire_status != SEC_BATTERY_CABLE_NONE)
		{
			...
			wireless_current = wireless_current * (is_hv_wireless_type(temp_current_type) ? SEC_INPUT_VOLTAGE_9V : SEC_INPUT_VOLTAGE_5V);
			if (battery->wire_status == SEC_BATTERY_CABLE_PDIC)
			{
			...
			{
				wire_current = (battery->wire_status == SEC_BATTERY_CABLE_PREPARE_TA ? battery->pdata->charging_current[SEC_BATTERY_CABLE_TA].input_current_limit : battery->pdata->charging_current[battery->wire_status].input_current_limit);

				wire_current = wire_current * (is_hv_wire_type(battery->wire_status) ? (battery->wire_status == SEC_BATTERY_CABLE_12V_TA ? SEC_INPUT_VOLTAGE_12V : SEC_INPUT_VOLTAGE_9V)
	...
	if (battery->wc_status)
	{
		...
		val.intval = is_wired_type(current_cable_type) ? MFC_LDO_OFF : MFC_LDO_ON;
		psy_do_property(battery->pdata->wireless_charger_name, set,
	...
	const char *buf, size_t count)
{
	...
	case SIOP_EVENT:
		if (sscanf(buf, "%10d\n", &x) == 1)
		{
			...
			{ // To reduce call noise with battery pack
	...
	case TEST_CHARGE_CURRENT:
		if (sscanf(buf, "%10d\n", &x) == 1)
		{
			if (x >= 0 && x <= 2000)
			{
				...
				battery->pdata->charging_current[SEC_BATTERY_CABLE_USB].input_current_limit = x;
				battery->pdata->charging_current[SEC_BATTERY_CABLE_USB].fast_charging_current = x;
				if (x > 500)
				{
	...
	case HMT_TA_CHARGE:
		if (sscanf(buf, "%10d\n", &x) == 1)
		{
			...
			if (x && (battery->cable_type != SEC_BATTERY_CABLE_NONE) && (battery->cable_type != SEC_BATTERY_CABLE_OTG))
			{
	...
	{
		struct cisd *pcisd = &battery->cisd;
		int temp_data[CISD_DATA_MAX_PER_DAY] = {
			0,
		};
								...
								const union power_supply_propval *val)
{
...
#if !defined(CONFIG_CCIC_NOTIFIER)
		if ((battery->muic_cable_type != ATTACHED_DEV_SMARTDOCK_TA_MUIC) && ((current_cable_type == SEC_BATTERY_CABLE_SMART_OTG) ||
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_HV_DISABLE:
			sec_bat_set_current_event(battery, (val->intval == CH_MODE_AFC_DISABLE_VAL ? SEC_BAT_CURRENT_EVENT_HV_DISABLE : 0), SEC_BAT_CURRENT_EVENT_HV_DISABLE);
			break;
		default:
			return -EINVAL;
		}
									 ...
									 union power_supply_propval *val)
{
	...
	{
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = is_wireless_type(battery->cable_type) ? 1 : 0;
		break;
	case POWER_SUPPLY_PROP_PRESENT:
									 ...
									 const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL:
		...
		if (is_hv_wireless_type(battery->cable_type))
		{
			...
			mutex_lock(&battery->iolock);
			value.intval = (val->intval) ? WIRELESS_VOUT_5V : battery->wpc_vout_level;
			psy_do_property(battery->pdata->wireless_charger_name, set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			battery->aicl_current = 0; /* reset aicl current */
			mutex_unlock(&battery->iolock);
...
static int make_pd_list(struct sec_battery_info *battery)
{
	...
	int pd_list_index = 0;
	int pd_charging_charge_power = battery->current_event & SEC_BAT_CURRENT_EVENT_HV_DISABLE ? battery->pdata->nv_charge_power : battery->pdata->pd_charging_charge_power;
	...
	battery->max_charge_power = battery->pdic_info.sink_status.power_list[battery->pd_list.pd_info[pd_list_index - 1].pdo_index].max_voltage *
								battery->pdic_info.sink_status.power_list[battery->pd_list.pd_info[pd_list_index - 1].pdo_index].max_current / 1000;
							...
							struct sec_battery_info *battery)
{
	...
	if (pdata->slave_thermal_source)
	{
		...
		pdata->slave_chg_temp_adc_table =
			kzalloc(sizeof(sec_bat_adc_table_data_t) *
						pdata->slave_chg_temp_adc_table_size,
					GFP_KERNEL);
	...
	{
		pdata->swelling_drop_voltage_condition = (pdata->swelling_drop_float_voltage > 10000) ? (pdata->swelling_drop_float_voltage / 10) : (pdata->swelling_drop_float_voltage);
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	battery->polling_count = 1; /* initial value = 1 */
	battery->polling_time = pdata->polling_time[SEC_BATTERY_POLLING_TIME_DISCHARGING];
	battery->polling_in_sleep = false;
	battery->polling_short = false;
