...
static int displayport_remove(struct platform_device *pdev)
{
	...
	rt_mutex_destroy(&displayport->cmd_lock);
	rt_mutex_destroy(&displayport->hpd_lock);
	rt_mutex_destroy(&displayport->aux_lock);
	rt_mutex_destroy(&displayport->training_lock);
...
Voltage_Swing_Retry:
	...
	if (!(drive_current[0] == 3 && drive_current[1] == 3 && drive_current[2] == 3 && drive_current[3] == 3))
	{
...
static int displayport_fast_link_training(void)
{
	...
	if (lane_cnt == 0x04)
	{
		if ((lane_channel_eq_done == 0x0F) && (lane_symbol_locked_done == 0x0F) && (interlane_align_done == 1))
		{
			displayport_reg_set_training_pattern(NORAMAL_DATA);
	...
	{
		if ((lane_channel_eq_done == 0x03) && (lane_symbol_locked_done == 0x03) && (interlane_align_done == 1))
		{
			displayport_reg_set_training_pattern(NORAMAL_DATA);
	...
	{
		if ((lane_channel_eq_done == 0x01) && (lane_symbol_locked_done == 0x01) && (interlane_align_done == 1))
		{
			displayport_reg_set_training_pattern(NORAMAL_DATA);
...
static int displayport_link_training(void)
{
	...
	rt_mutex_lock(&displayport->training_lock);
	...
	rt_mutex_unlock(&displayport->training_lock);
											  ...
											  u8 lane2_3_status, u8 lane_align_status)
{
	...
	if (lane_cnt >= 1)
	{
		if ((lane0_1_status & (LANE0_CR_DONE | LANE0_CHANNEL_EQ_DONE | LANE0_SYMBOL_LOCKED)) != (LANE0_CR_DONE | LANE0_CHANNEL_EQ_DONE | LANE0_SYMBOL_LOCKED))
		{
	...
	if (lane_cnt >= 2)
	{
		if ((lane0_1_status & (LANE1_CR_DONE | LANE1_CHANNEL_EQ_DONE | LANE1_SYMBOL_LOCKED)) != (LANE1_CR_DONE | LANE1_CHANNEL_EQ_DONE | LANE1_SYMBOL_LOCKED))
		{
	...
	if (lane_cnt == 4)
	{
		if ((lane2_3_status & (LANE2_CR_DONE | LANE2_CHANNEL_EQ_DONE | LANE2_SYMBOL_LOCKED)) != (LANE2_CR_DONE | LANE2_CHANNEL_EQ_DONE | LANE2_SYMBOL_LOCKED))
		{
			displayport_err("%s() lane2 stat error\n", __func__);
			return -EINVAL;
		}

		if ((lane2_3_status & (LANE3_CR_DONE | LANE3_CHANNEL_EQ_DONE | LANE3_SYMBOL_LOCKED)) != (LANE3_CR_DONE | LANE3_CHANNEL_EQ_DONE | LANE3_SYMBOL_LOCKED))
		{
...
static int displayport_check_hdcp_version(void)
{
	...
	if ((((rx_caps & VERSION) >> DPCD_HDCP_VERSION_BIT_POSITION) == (HDCP_VERSION_2_2)) && ((rx_caps & HDCP_CAPABLE) != 0))
	{
#if defined(HDCP_2_2)
...
static int displayport_disable(struct displayport_device *displayport)
{
	...
	rt_mutex_lock(&displayport->cmd_lock);
	displayport->state = DISPLAYPORT_STATE_OFF;
	hdcp13_info.auth_state = HDCP13_STATE_NOT_AUTHENTICATED;
	rt_mutex_unlock(&displayport->cmd_lock);
...
{
	if (displayport->ven_id == SAMSUNG_VENDOR_ID && displayport->prod_id == DEXDOCK_PRODUCT_ID)
		return true;
...
static int displayport_probe(struct platform_device *pdev)
{
	...
	rt_mutex_init(&displayport->cmd_lock);
	rt_mutex_init(&displayport->hpd_lock);
	rt_mutex_init(&displayport->aux_lock);
	rt_mutex_init(&displayport->training_lock);
