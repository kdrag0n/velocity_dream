...
static void __decon_dump(bool en_dsc, u32 dump_lv)
{
	...
	if (decon->lcd_info->dsc_enabled && en_dsc && (dump_lv == DPU_DUMP_LV0))
	{
...
/* ---------- CHECK FUNCTIONS ----------- */
static void decon_win_conig_to_regs_param(int transp_length, struct decon_win_config *win_config,
...
static int decon_acquire_window(struct decon_device *decon, int idx)
{
	...
	if (((decon->dt.out_type == DECON_OUT_DSI) || (decon->dt.out_type == DECON_OUT_DP)) && (idx == decon->dt.dft_win))
	{
...
static int decon_probe(struct platform_device *pdev)
{
	...
	rt_mutex_init(&decon->vsync.lock);
	rt_mutex_init(&decon->fsync.lock);
	rt_mutex_init(&decon->lock);
	rt_mutex_init(&decon->pm_lock);
	rt_mutex_init(&decon->up.lock);
...
#endif
	if (decon->id != 2)
	{ /* for decon2 + displayport start in winconfig. */
