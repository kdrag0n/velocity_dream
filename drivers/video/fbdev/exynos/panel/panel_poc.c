...
static u8 poc_wr_img[POC_IMG_SIZE] = {
	0,
};
static u8 poc_rd_img[POC_IMG_SIZE] = {
	0,
};
#ifdef POC_DEBUG
static u8 poc_txt_img[POC_IMG_SIZE * 6] = {
	0,
};
#endif
...
int poc_img_backup(struct panel_device *panel, int size)
{
	...
	while (pos < size)
	{
		len = ((size - pos) < POC_IMG_WR_SIZE) ? (size - pos) : POC_IMG_WR_SIZE;
		poc_info->rpos = pos;
		poc_info->rsize = len;
