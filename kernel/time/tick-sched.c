...
static u64 tick_nohz_next_event(struct tick_sched *ts, int cpu)
{
...
static void tick_nohz_stop_tick(struct tick_sched *ts, int cpu)
{
	...
	if (ts->tick_stopped && (expires == ts->next_tick.tv64)) {
		...
		if (tick.tv64 == KTIME_MAX || ts->next_tick.tv64 == hrtimer_get_expires(&ts->sched_timer).tv64)
			return;
...
ktime_t tick_nohz_get_sleep_length(ktime_t *delta_next)
{
	...
	*delta_next = ktime_sub(dev->next_event, now);
	...
	next_event.tv64 = tick_nohz_next_event(ts, cpu);
	if (!next_event.tv64)
		return *delta_next;
	 ...
	 */
	next_event.tv64 = min_t(u64, next_event.tv64,
